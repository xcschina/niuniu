<?php
COMMON('adminBaseCore','luoke_qb/config','luoke_qb/luoke_tools');
class luoke_services extends adminBaseCore {
    ...
    public function queryItems($game_name){
        ...
        $sign_params = array(
            'merchant_id'=>config::B_MERCHANTID,
            'timestamp'=>$timestamp,
            'format'=>'Json',
            'version'=>'1.0',
            'game_name'=>$game_name
        );
        $sign = luoke_tools::set_sign($sign_params);
        $url_params = array(
            'merchant_id'=>config::B_MERCHANTID,
            'sign'=>$sign,
            'timestamp'=>$timestamp,
        ...
        $url = config::B_ENVIRONMENT.'/merchantgame/queryItems?'.http_build_query($url_params);
    ...
    public function chargeQbi($order_id,$item_id,$game_name,$account,$buy_no){
        ...
        $url = config::B_ENVIRONMENT.'/merchantgame/chargeQbi?'.http_build_query($url_params);
        return json_decode($this->request($url),true);
    }
    ...
    public function queryOrder($order_id){
        ...
        $url = config::B_ENVIRONMENT.'/merchantgame/queryOrder?'.http_build_query($url_params);
        return json_decode($this->request($url),true);
    }
    ...
    public function queryBalance(){
        ...
        $url = config::B_ENVIRONMENT.'/merchantgame/queryBalance?'.http_build_query($url_params);
        return json_decode($this->request($url),true);
    }
    ...
    public function queryDailyTrade($trade_date){
        ...
        $url = config::B_ENVIRONMENT.'/merchantgame/queryDailyTrade?'.http_build_query($url_params);
        return json_decode($this->request($url),true);
    }
    ...
    public function queryRechargeList($start_time,$end_time=''){
        ...
        $url = config::B_ENVIRONMENT.'/merchantgame/queryRechargeList?'.http_build_query($url_params);
        return json_decode($this->request($url),true);
    }
