...
class imageUtils {
	...
	// 原文件,新文件,宽度,高度,维持比例,是否自动加后缀名
	public static function resizeImage($big_image_name, $new_name, $max_width = 400, $max_height = 400, $resize = 1, $append_ext=0){
		$returnr['file']='';
		$returnr['filetype']='';

		//	$temp_img_type = @getimagesize($big_image_name);
		//	print_r($temp_img_type);
		//	exit();
		...
		{
			$func_output($big_image, ($append_ext ? $new_name.$func_exname : $new_name));
			$returnr['file']=($append_ext ? $new_name.$func_exname : $new_name);
		...
		$func_output($temp_image, ($append_ext ? $new_name.$func_exname : $new_name));
	 ...
	 */
	public static function imageWaterMark($groundImage,$waterPos=0,$waterImage="",$waterText="",$textFont=5,$textColor="#FF0000",$myfontpath="../data/mask/cour.ttf",$w_pct,$w_quality){
		global $fun_r;
		$isWaterImage = FALSE;
