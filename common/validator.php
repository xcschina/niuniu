...
class ValidFluent{
    ...
    private static $error_required = '该项为必填项目';
    private static $error_date = '请输入正确的日期格式,如:2015-10-10';
    private static $error_email = '邮箱格式不正确';
    private static $error_url = '请输入正确的网址格式';
    ...
    private function setErrorMsg($errorMsg, $default, $params=NULL){
		...
		if ($errorMsg == ''){
            ...
            $this->message[] = sprintf($default, $params);
		}else{
    ...
    function numberRic($errorMsg = Null){
        if ($this->isValid && (!empty($this->currentObj->value))) {
            $this->isValid = is_numeric($this->currentObj->value) ? TRUE : FALSE;
            if (!$this->isValid){
                $this->setErrorMsg($errorMsg, self::$error_numberInteger);
            }
        }
    ...
    function numberInt($errorMsg=NULL){
        if ($this->isValid && (!empty($this->currentObj->value))) {
            $this->isValid = (preg_match($this->pattern_numberInteger, $this->currentObj->value)) ? TRUE : FALSE;
            if (!$this->isValid || !is_int($this->currentObj->value))
                $this->setErrorMsg($errorMsg, self::$error_numberInteger);
        }
        return $this;
    }
	...
	function sameValue($same_value, $errorMsg){
		if ($this->isValid && (!empty($this->currentObj->value))) {
			$this->isValid = $this->currentObj->value == $same_value ? TRUE : FALSE;
			if (!$this->isValid)
				$this->setErrorMsg($errorMsg, self::$error_numberInteger);
		}
		return $this;
