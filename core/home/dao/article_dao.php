...
class article_dao extends Dao{
   ...
   //获取模块列表
    ...
    public function get_prve_article_info($id, $part_id=1, $game_id=null){
        $data = memcache_get($this->mmc,'wx_prve_artilce_info'.$id);
        if(!$data){
            $this->sql="select id,title from articles where is_pub=1 and id<? and part_id=? and game_id=? order by id desc limit 1";
            $this->params=array($id,$part_id,$game_id);
            $this->doResult();
            $data = $this->result;
            memcache_set($this->mmc, 'wx_prve_artilce_info'.$id, $data, 1, 3600);
        }
        return $data;
    }

    public function get_next_article_info($id, $part_id=1, $game_id=null){
        $data = memcache_get($this->mmc,'wx_next_artilce_info'.$id);
        if(!$data){
             $this->sql="select id,title from articles where is_pub=1 and id>? and part_id=? and game_id=? order by id asc limit 1";
            $this->params=array($id,$part_id,$game_id);
            $this->doResult();
            $data = $this->result;
            memcache_set($this->mmc, 'wx_next_artilce_info'.$id, $data, 1, 3600);
        }
        return $data;
    }
