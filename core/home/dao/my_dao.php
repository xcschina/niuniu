...
class my_dao extends Dao{
    ...
    public function get_isread_msg_count($user_id){
        $this->sql = "select count(1) as isread_num from messages where is_read = 1 and receiver_id = ?";
        $this->params = array($user_id);
        $this->doResult();
        return $this->result['isread_num'];
    }
