...
class integral_mobile extends baseCore{
    ...
    public function index_view($app_id,$channel){
        $user_id = $_SESSION['usr_info']['user_id'];
        $user_info = $this->DAO->get_user_info($user_id);
        $app_message = $this->DAO->get_app_message($app_id);
        if($app_message){
            $app_info = $this->DAO->get_app_info($app_id);
        }else{
            $app_info = $this->DAO->get_apps($app_id);
        }
        if(!$app_info['app_id']){
            die("链接出错啦");
        }
...
//            $usr_info = $this->DAO->get_user_info($usr_id);
    ...
    public function exchange(){
        ...
        $user_message = $this->DAO->get_user_message($_SESSION['usr_info']['user_id']);
        if($user_message){
            $user_info = $this->DAO->get_user_info($_SESSION['usr_info']['user_id']);
        }else{
            $user_info = $this->DAO->get_user($_SESSION['usr_info']['user_id']);
        }
        if(!$user_info['mobile']){
            $result['result'] = 3;
            $result['desc'] = '请先绑定手机';
