...
class account_dao extends Dao{
    ...
    public function  get_login_ip_count($ip){
        $this->sql="select count(user_id) as num from user_info where  reg_ip=?";
    ...
    public function insert_user_info($password, $mobile, $reg_ip, $qq_id=''){
        ...
        if(!empty($_SESSION['promoter_id'])){
            $user_info = array("user_id"=>$user_id,"guid"=>$guid,"reg_time"=>strtotime("now"),"reg_ip"=>$reg_ip,"qq_id"=>$qq_id,"buy_mobile"=>$mobile,"reg_from"=>2,"code"=>$_SESSION['promoter_id']);
        }else{
    ...
    public function update_user_psw($password,$mobile,$user_id,$token){
        ...
        if($user_info['login_name']){
            $this->mmc->delete('account_info'.$_SESSION['user_id']);
        }
