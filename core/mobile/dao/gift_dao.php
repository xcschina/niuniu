...
class gift_dao extends Dao {
    ...
    public function get_gifts(){
        $this->sql = "select a.*,b.game_name,b.game_icon from game_gift_info as a INNER JOIN game as b on a.game_id=b.id
                        where a.is_del=0 and is_attach=0 and a.remain>0 and a.end_time>unix_timestamp(now()) order by a.is_top desc,a.lastupdate desc limit 50";
        $this->doResultList();
        return $this->result;
    }
    ...
    public function get_user_gift_batch($user_id,$batch_id){
        $this->sql = "select batch_id from weixin_gifts where user_id=? and batch_id=? group by batch_id";
