...
class login_mobile extends baseCore{
    public $DAO;
    ...
    public function do_login(){
        ...
        if(md5($pwd) != $usr_info['password']){
            ...
            $result['msg']  = "登录成功～";
            $result['code'] = 1;
            $result['url']  = $_SESSION['login_back_url'];
            die('0' . base64_encode(json_encode($result)));
        }
    }
...
//        var_dump('shjkfwe');
    ...
    public function forget(){
        $this->page_hash();
        $this->display("moyu/findPassword.html");
    }


    public function register(){
        $this->page_hash();
        $this->display("moyu/register.html");
    ...
    public function do_register(){
        ...
        if(!isset($_SESSION['reg_core']) ||
            ...
            $_POST['sms_code']==""){
            $result['msg'] = $_SESSION['m_reg_error'] = '验证码错误';
            die('0' . base64_encode(json_encode($result)));
        }

        if(strtotime("now")-$_SESSION['last_send_time']>300){
            $result['msg'] = $_SESSION['m_reg_error'] = '验证码超时';
            die('0' . base64_encode(json_encode($result)));
        }
        ...
        if($usr_id){
            $result['code'] = 1;
            $result['url']  = '/login.php?act=login';
            $result['msg']  = '注册成功~';
            die('0'.base64_encode(json_encode($result)));
        }
    ...
    public function do_forget(){
        ...
        if(!isset($_SESSION['reg_core']) || $_SESSION['reg_core'] != $_POST['mobile'] . "_" . $_POST['sms_code'] ||
            $_POST['sms_code'] == ""){
        ...
        if(strtotime("now")-$_SESSION['last_send_time']>300){
            $result['msg'] = $_SESSION['fg_error'] = '验证码超时';
            die('0' . base64_encode(json_encode($result)));
        }
        $user_info = $this->DAO->get_user_by_mobile($_POST['mobile']);
        if(!$user_info){
    ...
    public function do_modify_psw(){
        ...
        if(!$error){
            ...
            $this->redirect("login.php?act=logout");
...
//        }
    ...
    public function do_modify_user(){
        ...
        $this->redirect("login.php?act=modify_user");
    ...
    public function register_sms(){
        ...
        if(isset($_SESSION['last_send_time'])){
            if($nowtime - $_SESSION['last_send_time'] < 18){
