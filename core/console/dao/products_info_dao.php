...
class products_info_dao extends Dao {
    ...
    public function add_product($params){
        $this->sql="insert into products(proportion,num,role_level,role_job,role_sex,`type`,title,sub_title,game_id,channel_id,serv_id,serv_name,stock,price,intro,user_id,add_time,end_time,is_pub)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $this->params=array($params['proportion'],$params['num'],$params['role_level'],$params['role_job'],$params['role_sex'],$params['type'],$params['title'],$params['sub_title'],$params['game_id'],$params['channel_id'],$params['serv_id'],$params['serv_name'],$params['stock'],$params['price'],
    ...
    public function update_products_status($params){
        $this->sql = "update products set is_pub=?,audit_time=?,end_time=? where id = ?";
        $this->params = array(1,time(),strtotime("+7 day"),$params['id']);
        $this->doExecute();
    }
}