...
class general_dao extends Dao
{
    ...
    public function insert_activity($params){
        $this->sql = "insert into general_tb(title,down_url,banner,bg_img,`type`,add_time,game_id,batch_name,top_img,middle_img,bottom_img) values(?,?,?,?,?,?,?,?,?,?,?)";
        $this->params = array($params['title'],$params['down_url'],$params['banner'],$params['bg_img'],$params['type'],time(),$params['game_id'],$params['batch_name'],$params['top_img'],$params['middle_img'],$params['bottom_img']);
        $this->doInsert();
    }
    ...
    public function insert_general($params,$info,$data,$id){
        $this->sql = "insert into general_tb(title,game_id,down_url,banner,bg_img,add_time,code,pid,top_img,middle_img,bottom_img,code_name) values(?,?,?,?,?,?,?,?,?,?,?,?)";
        $this->params = array($info['title'],$info['game_id'],$params['down_url'],$info['banner'],$info['bg_img'],time(),$data,$id,$info['top_img'],$info['middle_img'],$info['bottom_img'],$params['code_name']);
        $this->doInsert();
    }
    ...
    public function update_general_info($id,$params){
        $this->sql = "update general_tb set title=?,down_url=?,banner=?,bg_img=?,`type`=?,game_id=?,batch_name=?,top_img=?,middle_img=?,bottom_img=?  where id = ? ";
        $this->params = array($params['title'],$params['down_url'],$params['banner'],$params['bg_img'],$params['type'],$params['game_id'],$params['batch_name'],$params['top_img'],$params['middle_img'],$params['bottom_img'],$id);
        $this->doExecute();
    }

    public function update_general_msg($id,$params){
        $this->sql = "update general_tb set title=?,down_url=?,banner=?,bg_img=?,game_id=?,top_img=?,middle_img=?,bottom_img=?  where id = ?";
        $this->params = array($params['title'],$params['new_url'],$params['banner'],$params['bg_img'],$params['game_id'],$params['top_img'],$params['middle_img'],$params['bottom_img'],$id);
        $this->doExecute();
    }
    ...
    public function get_log_list($params,$page){
        ...
        if($params['pid']){
            $this->limit_sql = $this->limit_sql." and a.pid = ".$params['pid'];
        }
        $this->limit_sql = $this->limit_sql." order by a.id desc";
        $this->doLimitResultList($page);
        return $this->result;
    ...
    public function get_down_log($params,$id,$page){
        $this->limit_sql = "select * from general_down_log where relation_id = ".$id;
        if($params['ip']){
            $this->limit_sql = $this->limit_sql." and ip = '".$params['ip']."'";
        }
        if($params['start_time'] && $params['end_time']){
            $this->limit_sql = $this->limit_sql .=  " and add_time >= ".$params['start_time']." and add_time <= ".$params['end_time'];
        }else if($params['start_time'] && !$params['end_time']) {
            $this->limit_sql = $this->limit_sql .=  " and add_time >= ".$params['start_time'];
        } else if(!$params['start_time'] && $params['end_time']) {
            $this->limit_sql = $this->limit_sql .=  " and add_time <= ".$params['end_time'];
        }
        $this->doLimitResultList($page);
        return $this->result;
    }

    public function get_visit_log($params,$id,$page){
        $this->limit_sql = "select * from general_visit_log where  relation_id = ".$id;
        if($params['ip']){
            $this->limit_sql = $this->limit_sql." and ip = '".$params['ip']."'";
        }
        if($params['start_time'] && $params['end_time']){
    ...
    public function get_all_general_log_list($params){
        ...
        if($params['title']){
            $this->sql = $this->sql." and title like '%".$params['title']."%'";
        }
        if($params['pid']){
            $this->sql = $this->sql." and pid = ".$params['pid'];
        }
