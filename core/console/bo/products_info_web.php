...
class products_info_web extends baseCore{
    ...
    public function get_products_list(){
        ...
        $this->pageInfo($this->pageCurrent,50);
    ...
    public function do_products_add(){
        ...
        if(mb_strlen($params['title'],'utf8')>20){
            die(json_encode($this->error_msg("商品名称长度不能超过20")));
        }
...
//        }
        ...
        if($params['type'] == 5 && !$params['stock']){
            die(json_encode($this->error_msg("库存不能为空")));
        }
        if($params['type'] == 6 && !$params['stock']){
            die(json_encode($this->error_msg("库存不能为空")));
        }
        if($params['type'] == 4){
            if(!$params['role_level'] || !$params['role_job'] || !$params['role_sex']){
                die(json_encode($this->error_msg("请把等级职业性别填写好")));
            }

        }
        if($params['type'] == 5){
            if(!$params['num']){
                die(json_encode($this->error_msg("请输入游戏币数量")));
            }
            $params['proportion'] = intval($params['num']/$params['price']);
...
//        }
        ...
        if($params['type'] == 5){
            if(!$params['num'] || !$params['stock'] ){
                die(json_encode($this->error_msg("请输入数量和库存")));
            }
            $params['proportion'] = intval($params['num']/$params['price']);
        }
//        var_dump($params['proportion']);
//        if(!$_FILES['product_img']['tmp_name']){
...
//        }
   ...
   //通过发布商品
    public function products_audit($id){
        $this->assign("id", $id);
        $this->display("products_audit_view.html");
    }

    public function do_products_audit(){
        $params           = $_POST;
        $this->DAO->update_products_status($params);
        echo json_encode($this->succeed_msg("商品审核成功", "products_list"));
    }

    public function refuse($id){
        $this->assign("id", $id);
        $this->display("products_refuse_view.html");
    }
    ...
    public function do_refuse(){
        ...
        $params['status'] = 3;
        $this->DAO->update_products_status($params);
        echo json_encode($this->succeed_msg("商品拒绝成功", "products_list"));
    }
    ...
    public function products_del_view($id){
        if(!$id){
            die(json_encode($this->error_msg("商品ID出错啦")));
        }
        $this->assign('id',$id);
        $this->display("products_del_view.html");
    }
    ...
    public function do_del(){
        ...
        $this->DAO->del_product_info($id);
        die(json_encode($this->succeed_msg("商品删除成功", "products_list")));
    }
}