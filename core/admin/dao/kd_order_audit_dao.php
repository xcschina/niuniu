<?php
COMMON('niuniuDao');
class kd_order_audit_dao extends niuniuDao
{
    ...
    public function get_all_audit_list($page, $params){

        $this->limit_sql = "select a.*,b.app_name,b.channel,c.service_name from  qb_orders a left join business_apps b on a.app_id = b.app_id left join business_services c on a.app_id = c.app_id and a.service_id = c.service_id  where a.channel_type=2";
//        if($user_id){
...
//        }
    ...
    public function get_first_audit_list($page, $params){
        ...
        $this->limit_sql = "select a.*,b.app_name,b.channel,c.service_name,d.real_name,d.account,d.p1,d.p2 from  qb_orders a left join business_apps b on a.app_id = b.app_id left join business_services c on a.app_id = c.app_id and a.service_id = c.service_id left join admins d on a.operation_id = d.id where (a.status=1 or a.status=2 or a.status=8 or a.status=3  or a.status=4 or a.status=9) and a.channel_type=2";
//        if($user_id){
//            $this->limit_sql .= " and a.user_id in (".$user_id.")";
//        }
        if($params['financial_mode']){
    ...
    public function get_list($page, $params){
        ...
        $this->limit_sql = "select a.*,b.app_name,b.channel,c.service_name,d.real_name,d.account,d.p1,d.p2 from  qb_orders a left join business_apps b on a.app_id = b.app_id left join business_services c on a.app_id = c.app_id and a.service_id = c.service_id left join admins d on a.operation_id = d.id where (a.status=1 or a.status=4 or a.status=9 ) and a.channel_type=2";
//        if($user_id){
//            $this->limit_sql .= " and a.user_id in (".$user_id.")";
//        }
        if($params['financial_mode']){
    ...
    public function update_qb_order_out_money($params){
        $this->sql = "update qb_orders set is_out=? where id = ?";
        $this->params = array(1,$params['id']);
        $this->doExecute();
    }
