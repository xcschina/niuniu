<?php
COMMON('dao');
class jinglan_public_dao extends Dao{
    ...
    public function list_data($page,$params){
        $this->limit_sql = "select * from jinglan where `type`=2";
        if($params['order_id']){
    ...
    public function insert_order_by_query($order_id, $params, $jinglan_order_id,$status){
        $this->sql = "insert into jinglan(order_id,amount,qq,jinglan_order_id,add_time,operation_id,game_id,status,price,`type`)values(?,?,?,?,?,?,?,?,?,?)";
        $this->params = array($order_id, $params['amount'], $params['qq'], $jinglan_order_id, time(), $_SESSION['usr_id'],$params['game_id'],$status,$params['price'],2);
        $this->doInsert();
        return $this->LAST_INSERT_ID;
    }

    public function insert_order($order_id,$params,$jinglan_order_id){
        $this->sql = "insert into jinglan(order_id,amount, price, qq, jinglan_order_id, add_time, operation_id,game_id,`type`)values(?,?,?,?,?,?,?,?,?)";
        $this->params = array($order_id,$params['amount'], $params['price'], $params['qq'], $jinglan_order_id, time(), $_SESSION['usr_id'],$params['game_id'],2);
        $this->doInsert();
        return $this->LAST_INSERT_ID;
    }

    public function get_ip_session($ip){
        $data = $this->mmc->get("jinglan_public_".$ip);
        return $data;
    }

    public function set_ip_session($ip,$data){
        $this->mmc->set("jinglan_public_".$ip,$data,1,600);
    }

    public function get_list_all($params){
        $this->sql = "select * from jinglan where `type`=2";
        if($params['order_id']){
    ...
    public function get_qb_price(){
        $this->sql = "select * from qb_price_log where price and qb_type=2 order by add_time desc";
        $this->doResult();
        return $this->result;
    }
    ...
    public function insert_qb_price_log($params,$user_id){
        $this->sql = "insert into qb_price_log (price,operator_id,order_id,add_time,qb_type)values(?,?,?,?,?)";
        $this->params = array($params['new_price'],$user_id,$params['id'],time(),2);
        $this->doInsert();
        return $this->LAST_INSERT_ID;
    }

    public function get_order_amount($qq,$start_time){
        $this->sql = "select sum(amount) as amount from jinglan where qq = ? and status <=3 and `type`=2 and add_time >= ? and add_time <= ?";
