...
DAO('jinglan_public_dao');

class jinglan_public_admin extends adminBaseCore{
    public $DAO;
    public $province;
    public $price;
    ...
    public function __construct(){
        parent::__construct();
        $this->DAO = new jinglan_public_dao();
        $this->province = '四川';
        $this->price = 0.98;
    }
    ...
    public function list_view(){
        ...
        $page = $this->pageshow($this->page, "jinglan_public.php?act=list&");
        $this->assign("operation_list",$operation_list);
        $this->assign("params",$params);
        $this->assign("datalist", $list);
        $this->assign("page_bar", $page->show());
        $this->display('jinglan_public_list.html');
    }

    public function add_view(){
        $this->page_hash();
        $this->display('jinglan_public_add.html');
    }
    ...
    public function do_add(){
        ...
        $nowtime = time();
        if(isset($_SESSION['jinglan_qb_last_send_time'])){
            if($nowtime-$_SESSION['jinglan_qb_last_send_time']<6){
                $this->error_msg("请求太频繁，请稍后再试");
            }else{
                $_SESSION['jinglan_qb_last_send_time'] = $nowtime;
            }
        }else{
            $_SESSION['jinglan_qb_last_send_time'] = $nowtime;
        }
...
//        $url = 'http://123.56.242.212:25000/mch/placeOrder.do';//测试环境
        ...
        $data = array(
            'merAccount'=>JL_merchant_Account_P,
            'merUserAccount'=>'',
            'businessType'=>10,
            'merOrderNo'=>$order_id,
        ...
        $jinglan['sign'] = md5('data='.$jinglan['data'].'&key='.jinglan_key_P);
...
//        $result['data'] = json_decode(json_encode((json_decode($result['data']))),TRUE);
        ...
        $this->err_log(var_export($result, 1), "jinglan_public_result");
        if($result['resultCode'] == '0' || $result['http_status'] == '200'){
            ...
            if($order_query_res['resultCode'] == '0' && $order_query_res['data']['merOrderNo'] == $order_id){
                ...
                $this->err_log(var_export($order_query_res, 1), "jinglan_public_result");
...
//        $result['data'] = json_decode(json_encode((json_decode($result['data']))),TRUE);
    ...
    private function jinglan_excel_out($data){
        ...
        $title = iconv("UTF-8", "GB2312//IGNORE","净蓝Q币对公数据-".$str_now.'.xls');
    ...
    public function price_edit($id){
        if($id){
            ...
            if($user_info['group_id']==1 || $user_info['id']==136){
                ...
                $this->display("jinglan_public_price.html");
            }else{
                die("你没有此权限，请联系管理员！");
            }
        }else{
