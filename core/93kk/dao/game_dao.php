...
class game_dao extends niuniuDao
{
    ...
    public function get_channel_app_info($app_id,$channel){
        $data = $this->mmc->get("channel_app_info_".$app_id."_".$channel);
        if(!$data){
            $this->sql = "select * from `niuniu`.channel_apps where app_id=? and ch_code=? ";
            $this->params = array($app_id,$channel);
            $this->doResult();
            $data = $this->result;
            $this->mmc->set("channel_app_info_".$app_id."_".$channel, $data, 1 ,600);
        }
        return $data;
    }

    public function get_super_app_info($super_id,$channel){
        $data = $this->mmc->get("channel_super_app_info_".$super_id.'_'.$channel);
        if(!$data){
            $this->sql = "select * from `niuniu`.channel_apps where super_id =? and ch_code = ? ";
            $this->params = array($super_id,$channel);
            $this->doResult();
            $data = $this->result;
            $this->mmc->set("channel_super_app_info_".$super_id.'_'.$channel,$data,1,120);
        }
        return $data;
    }
