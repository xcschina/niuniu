...
class account_web  extends baseKuyoo{
    public $DAO;
    public $COMDAO;
    ...
    public function ky_login(){
        $data = array('msg' => 1, 'desc' => "网络异常,刷新后重新登录.");
        $moblie = $_POST['mobile'];
        $pwd = $_POST['password'];
    ...
    public function ajax_qqlogin_phone_bind(){
        die("服务器正在升级，暂时不能绑定手机号。");
        $error=false;
        ...
        if(!$error){
            ...
            $data = array('msg' => 1, 'desc' => $error_des);
            die(json_encode($data));
        }
    }
...
//        }
    ...
    public function register_sms(){
        $phone = $_POST['mobile'];
        if($_SESSION['c'] != strtoupper($_POST['v_code'])){
            $msg['res']="0";//1成功0失败
            $msg['msg']="图形码错误";
            die(json_encode($msg));
        }
...
//        $send_result = json_decode(base64_decode(substr($send_result, 1)));
        $send_result = $this->send_sms($phone,array($code),"124312");
    ...
    public function email_code(){
        ...
        $content = '<p style=\"line-height: 20px;\"><strong>亲爱的用户：</strong></p>
                    ...
                    <p style=\"line-height: 20px;\">&nbsp; &nbsp;感谢您对<a href=\"http://shouyou.kuyou.com\">酷游</a>的支持，祝您生活愉快！</p>
...
//        $msg['code']=$code;
    ...
    public function qq_login_view(){
        $config['appid'] = KY_QQ_APP_ID;
        $config['appkey'] = KY_QQ_APP_KEY;
        $config['callback'] = "http://shouyou.kuyoo.com/qq_callback.php";
        $o_qq = new oauth_qq($config);
        $o_qq->login();
    }

    public function qq_do_login(){
        die("服务器正在升级，暂时不能QQ登录。");
        $config['appid'] = KY_QQ_APP_ID;
        $config['appkey'] = KY_QQ_APP_KEY;
        $config['callback'] = "http://shouyou.kuyoo.com/qq_callback.php";
        $o_qq = new oauth_qq($config);
