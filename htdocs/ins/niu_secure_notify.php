...
$bo = new baseCore();
$bo->err_log($bo->client_ip()."\r".var_export($_POST,1),'nnb_alisdk_notify');
...
if($verify_result){
    ...
    BO('android_pay');
    $bo = new android_pay();
    $bo->nnb_ali_notify($trade_status, $order_id, $trade_no, $buyer_email);
}else{
    $bo->err_log($verify_result,'nnb_alisdk_notify');
    echo "fail";
}
?>