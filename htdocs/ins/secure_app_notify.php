...
$bo = new baseCore();
$bo->err_log($bo->client_ip()."\r".var_export($_POST,1),'aliapp_notify');
...
if($verify_result){
    ...
    BO('android_pay');
    $bo = new android_pay();
    $bo->aliapp_notify($trade_status, $order_id, $trade_no, $buyer_email);
}else{
    $bo->err_log($verify_result,'aliapp_notify');
    echo "fail";
}
?>