...
if(!$params['usr_id'] || !$params['app_id'] || !$params['player_id'] || !$params['serv_id'] ||  !$params['money_id']){
    ...
    $api->err_log(var_export($params,1),'nd_pay_cp_log');
}
try{
    $api->nd_pay();
}catch (Exception $e){
    $api->err_log(var_export($params,1),"nd-pay-android-secure-pay");
    $api->err_log(var_export($e,1),"nd-pay-android-secure-pay");
}
