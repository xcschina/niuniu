...
COMMON('niuniuDao');
$bo = new baseCore();
$bo->err_log($bo->client_ip().":".var_export($_POST,1),'gamesite_notify_');

$_POST['notify_data'] = $_REQUEST['notify_data'];

$alipay = new alipay_notify(HNYQ_ALI_MOBILE_partner, HNYQ_ALI_MOBILE_key, HNYQ_ALI_MOBILE_sec_id, HNYQ_ALI_MOBILE_input_charset); // 构造通知函数信息
        ...
        if ($status == 'TRADE_FINISHED' || $status == 'TRADE_SUCCESS') {
            ...
            if(!$dao->result){
                ...
                if($order['status']!=2){
                    ...
                    $dao->params = array($ali_order_id, 1, $buyer_email, strtotime($pay_time),4,$order_id);//4为93kk回调
                    $dao->doExecute();
                }
                echo "success";
            }
