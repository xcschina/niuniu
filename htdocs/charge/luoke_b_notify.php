...
DAO('index_dao');
$bo = new baseCore();
$bo->err_log($bo->client_ip().":".var_export($_POST,1),'luoke_b_notify');

if($_POST['order_status'] == 3){
    $dao = new index_dao();
    $dao->sql = "update luoke_b set status=1, luoke_order_id=?, callback_time=? where order_id=?";
    ...
    $dao = new index_dao();
    $dao->sql = "update luoke_b set status=?, luoke_order_id=?, callback_time=? where order_id=?";
