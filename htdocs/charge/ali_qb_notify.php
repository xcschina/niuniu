...
if ($verify_result){
    ...
    try{
        if ($status == 'TRADE_FINISHED' || $status == 'TRADE_SUCCESS') {
            $dao = new index_dao();
            $dao->sql = "select * from qb_order where order_id=?";
            $dao->params = array($order_id);
            $dao->doResult();
            $order = $dao->result;

            if(!$dao->result){
                $bo->err_log('失败','qb_alipay');
                echo "fail";
            }else{
                if($order['status']!=2){
                    //更新订单状态
                    $dao->sql = "update qb_order set channel_order_id=?,status=?,payer=?,pay_time=? where order_id=?";
                    ...
                    $bo->send_mail("2874759177@qq.com;3311363532@qq.com;252432349@qq.com;270772735@qq.com;5992025@qq.com",'单号'.$order_id."付款",'有人购买QB ，单号'.$order_id."，通道支付宝");
                }
                echo "success";
            }
        } else {
