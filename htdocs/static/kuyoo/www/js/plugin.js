    ...
    if(rel=="hot"){
        ...
        $.each(games,function(idx,item){
            if(item.first_letter == rel){
                $("#index_game_content_list li.idx_gs_list:last").append('<dl><dt><a href="\/game'+item.id+'"><img src="http:\/\/static.kuyoo.com\/'+item.game_icon+'" \/><\/a><\/dt><dd>' +
        ...
        $.each(games,function(idx,item){
            if(item.first_letter == rel){
                html_str+='<div class="item"><div class="fl item-img">'+
                    '<a href="\/game'+item.id+'\/character"><img src="http:\/\/static.kuyoo.com\/'+item.game_icon+'" width="86" height="86" /><\/a><\/div>' +
                    '<div class="fl">' +
...
function ini_search(){
    ...
    keyword_search = function (keyword, callback) {
        ...
        $.each(games,function(idx,item){
            ...
            $("ul.keyword-inner").append('<li><a href="\/game'+item.id+'"><img src="http:\/\/static.kuyoo.com\/'+item.game_icon+'" \/>'+item.game_name+'<\/a><\/li>');
        });
    };
