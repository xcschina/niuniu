...
$('#choose_product').on('change',function(event){
	...
	if($pid > 0){
		$.get("/product.php",{act:'valid_discount',id:$pid},function(data){
			// 渠道
			if(data.channels){
				$('#android_platform').html('<label class="icon_android"></label>');
				$('#ios_platform').html('<label class="icon_ios"></label>');
...
var selected_platform_fn = function($gid,$ch){
	...
	$.get("/product.php",{act:'channel_servs',id:$gid,ch:$ch},function(data){
		...
		if(data.length>0){
			...
			$options += '<option value="1">其他服务器</option>';
			for(var j in data){
...
function check_character(){
	...
	var serv_id = $('#choose_server').val();
	$("input[name='other_ser']").val();

	var other_ser = $("input[name='other_ser']").val();
	if(serv_id == '1' && $.trim(other_ser)==""){
		alert("请填写其他区服信息");
		$flag = false;
	}
	return $flag;
