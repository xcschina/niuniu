...
var init_choose_pt_fn = function(){
	...
	// $('#price_order').find('option:first').prop('selected','selected').end().prop('disabled',true);
}
...
var search_coin_fn = function($nowpage){
	...
	$.post($url,$('#buyfrm').serialize(),function(data){
		...
		$pagelist = $('#page');
		// $coinlist.slideUp('slow');
		// $pagelist.slideUp('slow');
		$coinlist.empty();
		$pagelist.empty();
		$coin = ''; // 数据
		$page = ''; // 分页
		if(data.pro){
			...
			$url = 'http://www.66173.cn/buy/'+$gid+'-coin.html';
			for(var $i in $pro){
		...
		// $coinlist.slideDown('slow');
		// $pagelist.slideDown('slow');
