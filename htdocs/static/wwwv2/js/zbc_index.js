$(function(){
            ...
            if(games.length>0){
                $.each(games,function(idx,item){
                    if(item.first_letter == rel && $game_num<21){
                        $game_num++;
                        $last_game_icon.append('<dl> <dt><a target="_blank" rel="noopener noreferrer" href="/games/'+item.id+'.html"><img src="http://cdn.66173.cn/'+item.game_icon+'"/></a></dt> <dd><a target="_blank" rel="noopener noreferrer" href="/games/'+item.id+'.html">'+item.game_name+'</a></dd> </dl>');
                    }
                })
            }else{
    ...
    // 实时交易
    jQuery(".index_first_screen_right").slide({mainCell:"dl",autoPlay:true,effect:"topMarquee",vis:7,interTime:50,trigger:"click"});
    ...
    $('.search_btn').on('click',function(){
        sch = $('.txt').val();
        window.location.href='http://www.66173.cn/index.php?act=check_sch&sch='+sch;
    });
