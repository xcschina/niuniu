$(function(){
    ...
    keyword_search = function (keyword, callback) {
        games = $.ajax({url:"http://www.66173.cn/index.php?act=keyword&keyword="+encodeURIComponent(keyword),async:false});
        ...
        $.each(games,function(idx,item){
            ...
            $("ul.keyword-inner").append('<li><a target="_blank" rel="noopener noreferrer" href="\/games/'+item.id+'.html"><img src="http:\/\/cdn.66173.cn\/'+item.game_icon+'" \/>'+item.game_name+'<\/a><\/li>');
        });
        if(games.length > 0){
            $('#keyword_box').show();
        }else{
        ...
        if($sch != ''){
            $.post("/index.php?act=check_sch_do",{sch:$sch},function(data){
                switch(data['ret']){
                    ...
                    case 'wrong': 
                    default: $msg = '验证失败，该账号不是66173首充号！如果输入错误请重新输入！';break;
                }
                $('#res_msg').html($msg);
            },'json');
        }else{ $('#res_msg').html('验证失败，该账号不是66173首充号！如果输入错误请重新输入！'); }
    });
