...
try{
    if($type=='text'){
        ...
        if($content){
            ...
            if(!$games){
                $msg = $wechat->makeText('不好意思，牛牛听不懂您的话。如果您在菜单栏找不到您需要的功能，可以联系客服QQ3311363532。');
            }else{
        ...
        if($event=='subscribe'){
            //$data = $bo->get_setting();
            //$msg = $wechat->makeText(htmlspecialchars_decode($data));
            $msg = $wechat->makeText("感谢你关注牛牛~找好玩的游戏，找游戏攻略，找游戏资讯就戳底部菜单！回复手机游戏名有彩蛋哦~");
        }elseif($key=='service'){
